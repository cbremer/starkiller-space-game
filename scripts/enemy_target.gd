...
@export_range(0.5, 2.5, 0.05) var air_sprite_scale := 1.18
@export_range(0.5, 2.5, 0.05) var ground_sprite_scale := 1.24
...
func _update_visual() -> void:
	...
	if target_type == "ground":
		_sprite.texture = PLACEHOLDER_TEXTURES.enemy_ground_texture()
		_sprite.scale = Vector2.ONE * ground_sprite_scale
		return

	_sprite.texture = PLACEHOLDER_TEXTURES.enemy_air_texture()
	_sprite.scale = Vector2.ONE * air_sprite_scale
