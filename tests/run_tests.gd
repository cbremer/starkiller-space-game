...
func _run_all() -> void:
	_run_test("Nova charging and lifecycle guards", _test_nova_lifecycle)
...
func _test_nova_lifecycle() -> void:
	var state = GameStateScript.new()
	_expect_true(not state.try_spend_nova(), "Nova blocked before start")
	state.start_run()
	state.add_score(500)
	_expect_eq(state.nova_charge, 50.0, "score charges Nova")
	_expect_true(not state.try_spend_nova(), "partial Nova cannot fire")
	state.add_score(1000)
	_expect_eq(state.nova_charge, 100.0, "charge capped")
	state.toggle_pause()
	_expect_true(not state.try_spend_nova(), "Nova blocked while paused")
	state.toggle_pause()
	state.die()
	_expect_true(not state.try_spend_nova(), "Nova blocked while dead")
	state.respawn()
	_expect_true(state.try_spend_nova(), "charged Nova fires after respawn")
	_expect_true(not state.try_spend_nova(), "Nova cannot fire twice")
	state.add_score(500, false)
	_expect_eq(state.nova_charge, 0.0, "Nova kills cannot recharge Nova")
	state.add_score(500)
	state.start_run()
	_expect_eq(state.nova_charge, 0.0, "restart clears charge")
